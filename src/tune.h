...
class Tune {
    ...
    // Our facilty to fill the container, each Entry corresponds to a parameter to
    // tune. We use variadic templates to deal with an unspecified number of
...
#define TUNE_CONDITIONS() \
    int UNIQUE(c, __LINE__) = (Conditions.init(__COUNTER__), 0); \
