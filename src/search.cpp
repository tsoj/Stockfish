...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (!PvNode && !excludedMove && ttData.depth > depth - (ttData.value <= beta) - (depth / 8)
        && is_valid(ttData.value)  // Can happen when !ttHit or when access race in probe()
        && (ttData.bound & (ttData.value >= beta ? BOUND_LOWER : BOUND_UPPER))
        && (cutNode == (ttData.value >= beta) || depth > 5 + (ss->ttPv ? depth / 4 : 0)))
    {
        ...
        if (pos.rule50_count() < 90 + (depth / 4))
        {
                ...
                && !is_decisive(ttData.value))
            {
                ...
                return (ttData.bound & BOUND_UPPER) && !cutNode ? (ttData.value + alpha) / 2
                                                                : ttData.value;
        }
    }
