    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if (depth >= 2 && moveCount > 1)
        {
            ...
            if (value > alpha && d < newDepth)
            {
                ...
                // Use 'alpha' as the reference score if no prior move has beaten it,
                // otherwise use 'bestValue'. This makes depth adjustments for the first
                // LMR success more conditional on the magnitude of its score over alpha.
                Value reference_score = (bestValue <= alpha) ? alpha : bestValue;

                const bool doDeeperSearch    = value > (reference_score + 42 + 2 * newDepth);
                const bool doShallowerSearch = value < reference_score + 9;
