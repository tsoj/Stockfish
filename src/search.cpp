...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if (depth >= 2 && moveCount > 1)
        {
            ...
            // Give quiet moves with very high history scores a slightly higher minimum LMR depth.
            int minLmrDepth = 1;
            if (!capture && ss->statScore > 6000)  // Threshold empirically determined
                minLmrDepth = 2;

            Depth d = std::max(minLmrDepth, std::min(newDepth - r / 1024,
