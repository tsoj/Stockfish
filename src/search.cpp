...
moves_loop:  // When in check, search starts here
    ...
    else if (!priorCapture && prevSq != SQ_NONE)
    {
        ...
        // Scale bonus based on how much the result undershoots static evaluation
        if (!ss->inCheck)
            bonusScale += std::clamp((ss->staticEval - bestValue) / 3, 0, 50);
        if (!(ss - 1)->inCheck)
            bonusScale += std::clamp((-(ss - 1)->staticEval - bestValue) / 3, 0, 40);
