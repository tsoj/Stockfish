    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if (depth >= 2 && moveCount > 1)
        {
            ...
            if (value > alpha && d < newDepth)
            {
                ...
                // If no best move found yet, be slightly more conservative about increasing depth,
                // unless the move is tactical or the fail-high margin is large.
                if (!bestMove && doDeeperSearch && !capture && !givesCheck && value <= alpha + 75)
