    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if (depth >= 2 && moveCount > 1)
        {
            ...
            if (value > alpha && d < newDepth)
            {
                ...
                // good enough search deeper. // Removed: if it was bad enough search shallower.
                const bool doDeeperSearch = value > (bestValue + 42 + 2 * newDepth);
                // const bool doShallowerSearch = value < bestValue + 9; // Removed condition

                newDepth += doDeeperSearch;  // Simplified adjustment
