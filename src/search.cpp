...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if (depth >= 2 && moveCount > 1)
        {
            ...
            // (*Scaler) Usually doing // Do full-depth search when reduced LMR search fails high
            if (value > alpha)
            {
                // Adjust full-depth search based on LMR results - if the result was
                // good enough search deeper, if it was bad enough search shallower.
                const bool doDeeperSearch = value > (bestValue + 35 + 3 * newDepth);
                const bool doMuchDeeperSearch =
                  value > (bestValue + 150 + 8 * newDepth) && !ss->inCheck && (PvNode || cutNode);
                const bool doShallowerSearch = value < bestValue + newDepth / 2;

                newDepth += doDeeperSearch + doMuchDeeperSearch - doShallowerSearch;
