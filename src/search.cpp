...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
        ...
        && pos.non_pawn_material(us) && ss->ply >= nmpMinPly && !is_loss(beta))
    {
        ...
        // Null move dynamic reduction based on depth and evaluation difference from beta
        int   evalDiff = std::max(ss->staticEval - beta, 0);
        Depth R        = 7 + depth / 3 + std::min(evalDiff / 256, 3);
        ...
        {
            if (nmpMinPly || (depth < 14 && nullValue < beta + 500))
                return nullValue;
