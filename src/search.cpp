...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (!allNode && depth >= 6 && !ttData.move)
    {
        ...
        // Apply additional reduction for deeper searches when position looks unpromising
        if (depth >= 12 && eval + 78 * depth <= alpha)
            depth--;

        // Cut nodes get extra reduction at very high depths
        if (cutNode && depth >= 16)
            depth--;
    }
