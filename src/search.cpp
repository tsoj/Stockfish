...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if (depth >= 2 && moveCount > 1)
        {
            ...
            if (value > alpha)
            {
                // Skip verification search if value significantly exceeds beta
                const Value skipMargin = std::max(10, 50 - depth * depth / 50);
                if (value - beta >= skipMargin)
                {
                    // Skip verification search - move is clearly winning
                }
                else
                {
