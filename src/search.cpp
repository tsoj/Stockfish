...
moves_loop:  // When in check, search starts here
    ...
    // Update correction history only in PV nodes with exact scores and quiet moves
    if (PvNode && bestMove != Move::none() && bestValue < beta)
    {
        if (!ss->inCheck && !pos.capture(bestMove))
        {
