...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
        ...
        && ss->ply >= thisThread->nmpMinPly && !is_loss(beta))
    {
        ...
        // Reduce R slightly (search deeper) if the previous move was a capture,
        // as the position might be more tactically volatile.
        Depth R = std::min(int(eval - beta) / 213, 6) + depth / 3 + 5 - priorCapture;
