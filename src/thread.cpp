...
/// Thread::bestMoveCount(Move move) return best move counter for the given root
/// move
...
void ThreadPool::start_thinking(Position&                 pos,
    ...
    // we need to backup and later restore setupStates->back(). Note that
    // setupStates is shared by threads but is accessed in read-only mode.
